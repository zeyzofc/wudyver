  ...
  try {
    ...
    const getUrlFromData = videoData => {
      if (videoData.edge_sidecar_to_children) {
        return videoData.edge_sidecar_to_children.edges.map(edge => edge.node.video_url || edge.node.display_url);
      }
...
async function saveIG(query) {
  try {
    ...
    const [response] = await Promise.all([axiosInstance.post("https://saveig.app/api/ajaxSearch", qs.stringify(requestData))]);
...
async function instagramDownloader(text) {
  const methods = [async () => {
    try {
